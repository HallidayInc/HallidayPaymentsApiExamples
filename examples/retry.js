document.addEventListener('DOMContentLoaded', async () => {
    ...
    */
    ...
    for (let i = 0; i < payments.length; i++) {
      ...
      li.innerHTML = `
        ...
        <button class="small-button">Retry</button>
      `;
      ...
      li.querySelector('.small-button').addEventListener('click', async () => {
        ...
        const paymentInfo = { paymentId: payment.payment_id, type, status, input, output, outputAsset, provider, amount, time }
  ...
  async function displaySigningPage(paymentInfo, balances) {
    const { paymentId, type, status, input, output, outputAsset, provider, amount, time } = paymentInfo;
    ...
    for (let i = 0; i < balances.balance_results.length; i++) {
      ...
      item.innerHTML = `
        <div class="token-name">Fees: $${fees}</div>
        <div class="token-amount">Output Amount: ${outputAmount} ${output}</div>
        <div id="status" class="token-amount">Status: ${status}</div>
        <div class="transaction">
          <a target="_blank" class="hidden" href="">See Transaction</a>
        </div>
        <button class="small-button">Sign & Submit Retry</button>
      `;

      const retryButton = item.querySelector('.small-button');
      retryButton.addEventListener('click', async () => {
        ...
        let statusUpdate = setInterval(async () => {
          const _status = await getStatus(newPaymentId);
          statusElement.innerText = `Status: ${_status.status}...`;

          if (_status.status === 'COMPLETE') {
            statusElement.innerText = `Status: COMPLETE`;

            // Show the resulting transaction on the proper block explorer
